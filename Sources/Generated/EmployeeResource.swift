...
public class EmployeeResource {
    private let adapter: EmployeeAdapter
    private let path = "/api/employees"
    private let pathWithID = "/api/employees/:id"

    init(factory: AdapterFactory) throws {
        adapter = try factory.getEmployeeAdapter()
    }
    ...
    private func handleCreate(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) {
        ...
        do {
            let model = try Employee(json: json)

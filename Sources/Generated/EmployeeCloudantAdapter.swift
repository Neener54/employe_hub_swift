...
public class EmployeeCloudantAdapter: EmployeeAdapter {
    let client: CouchDBClient
    var database: Database {
        // TODO: Properly ensure database exists
        client.createDB("employee") { _, _ in }
        return client.database("employee")
    }
    ...
    public func findAll(onCompletion: @escaping ([Employee], Swift.Error?) -> Void) {
        database.retrieveAll(includeDocuments: true) { maybeDocuments, error in
            ...
            do {
                let models = try documents.map { _, document -> Employee in
                    ...
                    let model = try Employee(json: modelJSON)
    ...
    public func create(_ model: Employee, onCompletion: @escaping (Employee?, Swift.Error?) -> Void) {
    ...
    public func deleteAll(onCompletion: @escaping (Swift.Error?) -> Void) {
        client.deleteDB("employee") { error in
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (Employee?, Swift.Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: Employee, onCompletion: @escaping (Employee?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (Employee?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
